...
func check_response(t *testing.T, point []int, list [][]int, resp [][]int){
	if len(resp) ==0 {
		t.Errorf("Empty response")
	}

	if len(resp) != len(list) {
		t.Errorf("Unexpected lenght of response. Expected %d got %d", len(resp), len(list))
	}
...
func Test_provided1(t *testing.T){
	...
	check_response(t, point, list, resp)
}
